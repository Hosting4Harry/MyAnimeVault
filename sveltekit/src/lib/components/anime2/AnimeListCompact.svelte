...
<div class="space-y-2">
    {#each animeList as anime (anime.id)}
        <div class="bg-white rounded-lg shadow p-3 flex items-center justify-between hover:bg-gray-50 transition">
            <div class="flex-1 mr-4">
                <div class="flex items-center space-x-2">
                    <h3 class="text-base font-semibold">{anime.title}</h3>
                    <span class={`px-2 py-0.5 text-xs rounded-full ${getStatusColor(anime.status)}`}>
